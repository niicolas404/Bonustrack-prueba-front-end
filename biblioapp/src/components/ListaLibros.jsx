function ListaLibros({ libros }) {
  return (
    <section className="book-list">
      {libros.map((libro) => (
        <article key={libro.id} className={`book-card ${libro.esNovedad ? 'book-card--new' : ''}`}>
          {libro.esNovedad && <span className="book-card__badge book-card__badge--nuevo">Nuevo</span>}
